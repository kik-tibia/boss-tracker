...
class RaidPredictorTest extends FunSuite {
  ...
  test("instantaneous chance is calculated correctly for a raid in its first day") {
    ...
    val expected = Some((3600 * 6 + 3600 * 20 * 4) / 0.75)
    val chance = RaidPredictor
      .calculateInstantaneousChance(raidType, raidStart).map(_.chance)
    assertOptionDouble(chance, expected)
  }
  ...
  test("instantaneous chance is calculated correctly for a raid with a two day window") {
    val raidType =
      defaultRaidTypeDto.copy(
        windowMin = Some(1),
        windowMax = Some(2),
    ...
    val expected = Some((86400 - 3600) * 2)
    val chance = RaidPredictor
      .calculateInstantaneousChance(raidType, raidStart).map(_.chance)
    assertOptionDouble(chance, expected)
  }
  ...
  test("a raid with a windowMin of 0 can't happen twice in one SS") {
    val raidType =
      defaultRaidTypeDto.copy(
        windowMin = Some(0),
        windowMax = Some(2),
    ...
    val raidStart = OffsetDateTime.of(2025, 6, 2, 22, 0, 0, 0, ZoneOffset.UTC)
    val expected = None
    val chance = RaidPredictor
      .calculateInstantaneousChance(raidType, raidStart).map(_.chance)
    assertOptionDouble[Double](chance, expected)
